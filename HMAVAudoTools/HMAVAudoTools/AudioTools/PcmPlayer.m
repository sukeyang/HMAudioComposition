...
@interface PcmPlayer () {
//    AppBasicBlock mProgressCallBack;
//    NSTimerHelper *m_timer;
}
...
//    NSLog(@"stopDurationTimer---,m_timer = %@",m_timer);
//    if (m_timer) {
//        [m_timer invalidate];
//        m_timer = nil;
//    }
...
//    if (!m_timer) {
//        m_timer = [NSTimerHelper scheduledTimerWithTimeInterval:0.3
//                                                         target:self
...
//- (void)setUpProgressBackBlock:(AppBasicBlock)block {
//    mProgressCallBack = [block copy];
//}
...
//    CGFloat current =  self.player.currentTime / total;
//    if (mProgressCallBack) {
//        mProgressCallBack (@(current));
//    }
//}
...
//+ (NSInteger)fecthMP3DurationWithFilePath:(NSString *)filePath {
...
//    PcmPlayer *voiceDurationPlayer = [[PcmPlayer alloc] initWithFilePath:filePath sampleRate:[instance.sampleRate integerValue]];
//    [voiceDurationPlayer play];
//    NSInteger allTime = ceil(voiceDurationPlayer.player.duration);
//    [voiceDurationPlayer.player stop];
//    voiceDurationPlayer.player = nil;
//     NSLog(@"音频持续时间是%f",voiceDurationPlayer.player.duration);
//    return allTime;
//}
@end
